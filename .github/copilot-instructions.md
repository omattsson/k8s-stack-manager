...
- **Resource quotas**: Per-cluster resource quotas (CPU, memory, storage, pods) enforced via Kubernetes ResourceQuota and LimitRange objects. Admin-configurable via API. Quota monitor emits `quota-warning` events.
...
| Dashboard | `/` | Stack instances overview with deploy/stop actions, widgets, setup wizard on first run |
| Templates | `/templates`, `/templates/new`, `/templates/:id`, `/templates/:id/edit`, `/templates/:id/use` | Gallery, builder, preview, version history, instantiate |
| Stack Definitions | `/stack-definitions`, `/stack-definitions/new`, `/stack-definitions/:id/edit` | List, create, edit definitions; import/export |
| Stack Instance | `/stack-instances/new`, `/stack-instances/:id`, `/stack-instances/compare` | Form, detail (values editor, branch selector, deploy preview, deploy/stop/clean/rollback), comparison |
...
| Notification Channels | `/admin/notification-channels` | Outbound webhook channels, subscriptions, delivery logs (admin only) |
...
`ROADMAP.md` tracks upcoming work. `ARCHITECTURE.md` describes the system; `EXTENDING.md` and `backend/docs/hooks.md` describe the hooks/actions extension contract. Security and scalability rules live in `.github/instructions/security.instructions.md` and `scalability.instructions.md`.
